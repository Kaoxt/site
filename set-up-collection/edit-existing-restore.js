(() => {
  ...
  function showInvalid(message) {
    ...
    if (status) { status.textContent = 'Invalid collection'; status.dataset.kind = 'error'; }
    const sync = $('#saveSetupBtn');
    if (sync) { sync.disabled = true; sync.textContent = 'Sync unavailable'; }
    const panel = $('#panelHost');
    if (!panel) return;
    const safe = String(message || '').replace(/[&<>"']/g, c => ({ '&':'&amp;', '<':'&lt;', '>':'&gt;', '"':'&quot;', "'":'&#039;' }[c]));
    panel.innerHTML = `<div class="panel"><div class="panel-head"><div class="kicker"><i></i>COLLECTION VERIFICATION</div><h2>Invalid collection</h2><p>${safe}</p></div><div class="card"><div class="callout warn"><strong>Editing is disabled.</strong> The Kollection only edits collection groups that match the IDs created by kollection.tv, so another creator's collection cannot be modified accidentally.</div><div class="actions"><a class="ghost" href="/account">Back to account</a><a class="btn" href="/set-up-collection">Set up The Kollection</a></div></div></div>`;
  }

  async function verifyOrigin() {
    try {
      const savedId = params.get('saved');
      const saved = await readJson(await fetch(`/api/account/collections/${encodeURIComponent(savedId)}`, { credentials: 'same-origin', cache: 'no-store' }));
      const item = saved?.collection || {};
      if (Number(item.draftStep || 0) < 7) { originVerified = true; active = true; return; }

      const expected = Array.isArray(item?.config?.selectedCollectionGroupIds)
        ? item.config.selectedCollectionGroupIds.map(collectionKey).filter(Boolean) : [];
      ...
      const response = await fetch(`${apiBase}/rest/v1/rpc/sync_pull_collections`, {
        method: 'POST',
        headers: { apikey: String(cfg.nuvioPublishableKey || ''), Authorization: `Bearer ${token.accessToken}`, 'Content-Type': 'application/json', Accept: 'application/json' },
        body: JSON.stringify({ p_profile_id: profileId }), cache: 'no-store',
      });
      const result = await response.json().catch(() => []);
      if (!response.ok) throw new Error(`Could not verify the Nuvio profile (HTTP ${response.status}).`);
      const rows = Array.isArray(result) ? result : [];
      const live = rows.length ? parseCollections(rows[0]?.collections_json) : [];
      const liveIds = new Set(live.map(group => collectionKey(group?.id)).filter(Boolean));
  ...
  function driveToCustomize() {
    ...
    if (step >= 4) {
      reachedCustomize = true; active = false;
      const status = $('#saveSetupStatus');
      if (status && /Loading saved setup|restor/i.test(status.textContent || '')) { status.textContent = 'Saved setup restored.'; status.dataset.kind = 'success'; }
      return;
    }
    if (step === 0) { clickOnce($('#startBtn')); return; }
    if (step === 1) {
      const profile = $('#profile'); const next = $('#nextBtn');
    ...
    if (step === 2) {
      ...
      const next = $('#nextBtn');
      if (builtIn) {
        const mdblist = $('#mdblist')?.value?.trim(); const host = $('#aiHost')?.value;
    ...
    if (step === 3) {
      ...
      if (customize && !customize.disabled) { clickOnce(customize, 80); return; }
      const next = $('#nextBtn');
  ...
  function init() {
    const panel = $('#panelHost');
    if (panel) new MutationObserver(() => setTimeout(driveToCustomize, 25)).observe(panel, { childList: true, subtree: true, characterData: true });
