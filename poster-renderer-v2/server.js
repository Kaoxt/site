...
async function dynamicAccent(imageBuffer) {
  try {
    const sample = await sharp(imageBuffer).resize(64, 64, { fit: 'cover' }).modulate({ saturation: 1.32, brightness: 0.94 }).stats();
    let { r, g, b } = sample.dominant;
    const max = Math.max(r, g, b, 1), min = Math.min(r, g, b);
    if (max - min < 22) { r *= 0.72; g *= 0.72; b *= 0.72; }
    else { const target = 138, scale = max > target ? target / max : 1; r *= scale; g *= scale; b *= scale; }
...
async function smartTopTag(text, { fill = '#191a20', fillOpacity = 0.94, width, fontSize = 35, textColor = '#ffffff' } = {}) {
  const resolvedWidth = width || smartTagWidth(text), safeText = esc(text), radius = 9;
...
async function originalBadgeImage(text, { width = 250, height = ORIGINAL_BADGE_HEIGHT, fill = '#101116', fillOpacity = 0.93, textColor = '#ffffff', fontSize = 44, radius = 16 } = {}) {
  const background = Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}"><rect x="1" y="1" width="${width-2}" height="${height-2}" rx="${radius}" fill="${fill}" fill-opacity="${fillOpacity}" stroke="#ffffff" stroke-opacity="0.14" stroke-width="2"/></svg>`);
...
async function smartBottomInfo(genre, ratingLabel) {
  ...
  const svg=Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="720" height="66" viewBox="0 0 720 66"><defs><filter id="shadow" x="-20%" y="-50%" width="140%" height="200%"><feDropShadow dx="0" dy="2" stdDeviation="2" flood-color="#000" flood-opacity="0.72"/></filter></defs><text x="360" y="33" text-anchor="middle" dominant-baseline="middle" font-family="DejaVu Sans" font-size="36" font-weight="700" letter-spacing="-0.25" fill="#c8c8cc" filter="url(#shadow)">${esc(text)}</text></svg>`);
  return sharp(svg).png().toBuffer();
}

async function titleImage(title){const text=String(title||'').trim();if(!text)return null;const fontSize=text.length>30?42:text.length>20?50:60;const shadow={text:{text:`<span foreground="#000000" alpha="78%" weight="bold">${esc(text)}</span>`,font:`DejaVu Sans ${fontSize}`,width:700,height:180,align:'center',rgba:true}},foreground={text:{text:`<span foreground="#ffffff" weight="bold">${esc(text)}</span>`,font:`DejaVu Sans ${fontSize}`,width:700,height:180,align:'center',rgba:true}};return sharp({create:{width:700,height:184,channels:4,background:{r:0,g:0,b:0,alpha:0}}}).composite([{input:shadow,top:4,left:1},{input:foreground,top:0,left:0}]).png().toBuffer();}
...
function smartBottomBackdrop(){return Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${POSTER_WIDTH}" height="450"><defs><linearGradient id="g" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#000" stop-opacity="0"/><stop offset="0.52" stop-color="#000" stop-opacity="0.10"/><stop offset="0.77" stop-color="#000" stop-opacity="0.31"/><stop offset="1" stop-color="#000" stop-opacity="0.66"/></linearGradient></defs><rect width="100%" height="100%" fill="url(#g)"/></svg>`);}
...
async function renderPoster(body){
 ...
 if(smartLayout){
  const dynamicFill=overlayColor==='dynamic'?await dynamicAccent(resized):overlayColor;composites.push({input:smartBottomBackdrop(),top:POSTER_HEIGHT-450,left:0});
  if(quality){if(trend){const t=await smartTopTag(trend,{fill:dynamicFill,width:smartTagWidth(trend,330,390)});composites.push({input:t.buffer,top:0,left:SAFE_MARGIN});}const q=await smartTopTag(quality,{fill:'#f3f4f6',fillOpacity:.96,width:smartTagWidth(quality,115,165),fontSize:31,textColor:'#111'});composites.push({input:q.buffer,top:0,left:POSTER_WIDTH-SAFE_MARGIN-q.width});}
  else if(trend){const t=await smartTopTag(trend,{fill:dynamicFill,width:smartTagWidth(trend,330,390)});composites.push({input:t.buffer,top:0,left:Math.round((POSTER_WIDTH-t.width)/2)});}
  if(age){const width=smartTagWidth(age,150,220),badge=await originalBadgeImage(age,{width,height:64,fill:'#111216',fillOpacity:.76,fontSize:34,radius:10});composites.push({input:badge,top:SMART_AGE_TOP,left:Math.round((POSTER_WIDTH-width)/2)});}
  const logo=await smartLogoImage(logoPath);if(logo){const left=Math.round((POSTER_WIDTH-logo.width)/2),top=SMART_LOGO_ZONE_TOP+Math.round(((SMART_LOGO_ZONE_BOTTOM-SMART_LOGO_ZONE_TOP)-logo.height)/2);composites.push({input:logo.buffer,top,left});}else if(title){const b=await titleImage(title);if(b)composites.push({input:b,top:800,left:40});}
