...
async function bottomInfoImage(text) {
  ...
  const shadow = {
    text: {
      text: `<span foreground="#000000" alpha="75%" weight="bold">${esc(text)}</span>`,
      font: 'DejaVu Sans 28',
      width: 700,
      height: 58,
      align: 'center',
  ...
  const foreground = {
    text: {
      text: `<span foreground="#e4e4e7" weight="bold">${esc(text)}</span>`,
      font: 'DejaVu Sans 28',
      width: 700,
      height: 58,
      align: 'center',
...
async function renderPoster(body) {
  ...
  const topTags = [];
  if (age) {
    const tag = await tagImage(age, { width: tagWidth(age, { min: 112, max: 170, perChar: 16 }), fill: '#191a20' });
    topTags.push(tag);
  }
  if (trend) {
    const tag = await tagImage(trend, {
      width: tagWidth(trend, { min: 184, max: 244, perChar: 15 }),
      fill: dynamicFill,
      fillOpacity: 0.94,
    });
    topTags.push(tag);
  ...
  if (quality) {
    const tag = await tagImage(quality, { width: tagWidth(quality, { min: 120, max: 190, perChar: 15 }), fill: '#191a20' });
    topTags.push(tag);
  }

  for (const placement of packTopTags(topTags)) {
...
const server = http.createServer(async (req, res) => {
  try {
    if (req.method === 'GET' && req.url === '/health') {
      ...
      return res.end(JSON.stringify({ ok: true, renderer: 'kollection-posters-v2-better-overlays-1' }));
    }
    ...
    res.writeHead(200, {
      ...
      'x-kollection-renderer': 'v2-better-overlays-1',
