...
const SAFE_MARGIN = px(30);

const SMART_TOP_HEIGHT = px(100);
const SMART_BOTTOM_INFO_TOP = px(1018);
...
function smartTagWidth(text, min = 220, max = 430) {
  return clamp(px(72 + String(text || '').length * 28), px(min), px(max));
}

async function smartTopTag(text, { fill = '#2f2d33', fillOpacity = 0.9, width, fontSize = 46, textColor = '#ffffff' } = {}) {
  const resolvedWidth = width || smartTagWidth(text), safeText = esc(text), radius = px(7), resolvedFontSize = px(fontSize);
  const svg = Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${resolvedWidth}" height="${SMART_TOP_HEIGHT}" viewBox="0 0 ${resolvedWidth} ${SMART_TOP_HEIGHT}"><path d="M0 0h${resolvedWidth}v${SMART_TOP_HEIGHT-radius}a${radius} ${radius} 0 0 1-${radius} ${radius}H${radius}A${radius} ${radius} 0 0 1 0 ${SMART_TOP_HEIGHT-radius}z" fill="${fill}" fill-opacity="${fillOpacity}"/><text x="${resolvedWidth/2}" y="${SMART_TOP_HEIGHT/2}" text-anchor="middle" dominant-baseline="middle" font-family="DejaVu Sans" font-size="${resolvedFontSize}" font-weight="700" letter-spacing="-0.22" fill="${textColor}">${safeText}</text></svg>`);
...
async function originalBadgeImage(text, { width = px(250), height = ORIGINAL_BADGE_HEIGHT, fill = '#101116', fillOpacity = 0.93, textColor = '#ffffff', fontSize = 44, radius = 16, strokeOpacity = 0.14 } = {}) {
  ...
  const textLayer = { text: { text: `<span foreground="${textColor}" weight="bold">${esc(text)}</span>`, font: `DejaVu Sans ${resolvedFontSize}`, width: Math.max(1,width-px(34)), height: Math.max(1,height-px(14)), align:'center', rgba:true } };
...
async function smartBottomInfo(genre, ratingLabel) {
  ...
  const width=px(720),height=px(84);
  const svg=Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><defs><filter id="shadow" x="-20%" y="-60%" width="140%" height="220%"><feDropShadow dx="0" dy="1" stdDeviation="1" flood-color="#000" flood-opacity="0.48"/></filter></defs><text x="${width/2}" y="${height/2}" text-anchor="middle" dominant-baseline="middle" font-family="DejaVu Sans" font-size="${px(58)}" font-weight="600" letter-spacing="-0.13" fill="#dedee3" fill-opacity="0.82" filter="url(#shadow)">${esc(text)}</text></svg>`);
  return sharp(svg).png().toBuffer();
}

async function titleImage(title){const text=String(title||'').trim();if(!text)return null;const fontSize=px(text.length>30?42:text.length>20?50:60),width=px(700),textHeight=px(180),canvasHeight=px(184);const shadow={text:{text:`<span foreground="#000000" alpha="68%" weight="bold">${esc(text)}</span>`,font:`DejaVu Sans ${fontSize}`,width,height:textHeight,align:'center',rgba:true}},foreground={text:{text:`<span foreground="#ffffff" weight="bold">${esc(text)}</span>`,font:`DejaVu Sans ${fontSize}`,width,height:textHeight,align:'center',rgba:true}};return sharp({create:{width,height:canvasHeight,channels:4,background:{r:0,g:0,b:0,alpha:0}}}).composite([{input:shadow,top:px(3),left:px(1)},{input:foreground,top:0,left:0}]).png().toBuffer();}
...
function smartBottomBackdrop(){const height=px(330);return Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${POSTER_WIDTH}" height="${height}"><defs><linearGradient id="g" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#000" stop-opacity="0"/><stop offset="0.48" stop-color="#000" stop-opacity="0.05"/><stop offset="0.76" stop-color="#000" stop-opacity="0.20"/><stop offset="1" stop-color="#000" stop-opacity="0.48"/></linearGradient></defs><rect width="100%" height="100%" fill="url(#g)"/></svg>`);}
...
export async function renderPoster(body){
 ...
 if(smartLayout){
  composites.push({input:smartBottomBackdrop(),top:POSTER_HEIGHT-px(330),left:0});
  ...
  if(age){const width=smartTagWidth(age,132,205),badge=await originalBadgeImage(age,{width,height:px(72),fill:'#111216',fillOpacity:.42,fontSize:38,radius:7,strokeOpacity:.22});composites.push({input:badge,top:SMART_AGE_TOP,left:Math.round((POSTER_WIDTH-width)/2)});}
  ...
  if(quality){const width=px(190);composites.push({input:await originalBadgeImage(quality,{width,height:px(92),fill:'#f3f4f6',fillOpacity:.9,textColor:'#111318',fontSize:44,radius:8,strokeOpacity:.05}),top:0,left:POSTER_WIDTH-SAFE_MARGIN-width});}
