...
For AIOMetadata custom poster patterns, generated Kollection configs now leave `usePosterProxy` off. AIOMetadata therefore places the resolved public `kollection.tv/api/posters-v2/...` URL directly in catalog/meta poster fields instead of wrapping it in AIOMetadata's `/poster-cache/proxy` route. This removes an application hop and lets Nuvio request Kollection's Cloudflare/R2 cache directly. The poster URL pattern and visual cache version are unchanged.
