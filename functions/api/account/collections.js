...
export async function onRequestPost(context) {
  try {
    ...
    return json({ collection: { id, name, draftStep, nuvioProfileId: profileId, nuvioProfileName: profileName, config: clean, createdAt: now, updatedAt: now } }, 201, auth.cookie ? { 'Set-Cookie': auth.cookie } : {});
  } catch (error) {
    console.error(error);
